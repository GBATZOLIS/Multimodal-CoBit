...
Its Gaussian corruption, binary score-matching objective, entropy-derived schedule, self-conditioning,
...
$D_\theta(\vx_\sigma,\sigma)\in(0,1)^S$ predicts the probability that each clean bit is one. For Gaussian corruption of binary data this
posterior mean determines the score, so the denoiser is trained by binary score matching,
implemented as the EDM-weighted denoising loss \citep{image:edm}
\begin{equation}
