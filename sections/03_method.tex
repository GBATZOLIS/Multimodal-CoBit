...
Every modality enters through one shared input projection, is processed by one transformer
backbone, is predicted by one bitwise prediction head and is trained under one denoising
objective; there are no modality-specific encoders, decoders, heads, embedding tables or losses,
and no modality-specific training stages. We recap the base model first, to the extent needed to make
...
for all of them. Consequently every position, whatever its modality, passes through the same
input projection, the same transformer backbone and the same bitwise prediction head, and is
trained under the same denoising objective; the shared Bernoulli output head costs
$O(m)=O(\log V)$ per position regardless of how many vocabularies are merged. The network learns
which modality a position holds from the bits themselves---the namespace or offset bits are part
of the $m$-bit word it reads---together with the fixed sequence template and 1-D position
described next. No modality- or segment-specific parameters are added.
