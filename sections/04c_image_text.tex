...
its trade-off are shown. CLIP is ViT-B/32, reported as $100\cdot\overline{\cos}$; per-direction guidance and churn are listed in
...
FID-comparable to CC3M's (CC3M at $n{=}5000$: 56.06 / 67.47 at 10 / 256 steps). The last two rows
are the floor of the FID column on the same unpaired estimator
(Appendix~\ref{app:image:protocol}): a perfect model scores 2.35 at this $n$ and 2.94 / 2.55
through the tokenizer, so the codec contributes 0.59 / 0.20.}
\label{tab:image:headline}
\end{table}

\textbf{Results.} Table~\ref{tab:image:headline} reports all four directions for both models,
each from a single set of weights. CC3M reaches zero-shot FID-30K \textbf{18.48} at NFE 20. Measured against a disjoint set of
real images, as that FID itself is, the floor of the column is 2.94, of which 2.35 is the
finite-sample floor any model pays at this $n$ and 0.59 the codec, so the gap to larger systems
lies in the model, not in the token space. Joint co-generation is much harder than either conditional direction (FID 50.19 at
...
\citet{image:geneval} (Table 2). Rows are not strictly comparable (see text): corpora,
backbones, pretrained components and FID pipelines all differ, and the same nominal metric varies
by about one FID point between sources (Stable Diffusion is 8.59 in one compilation and 9.62 in
another). Parameter counts follow each source's own convention and do not consistently include
frozen components (PixArt-$\alpha$'s 0.6B excludes a 4.3B T5 encoder, Imagen's 3B a frozen
T5-XXL, DALL$\cdot$E~2's 6.5B includes CLIP), so the column orders magnitude, not cost;
DALL$\cdot$E~2's prior, decoder and upsamplers saw 250M pairs, not the 650M of its CLIP encoder,
and CC3M is 3.3M pairs as released against the 2.9M we could download. CogView
\citep{image:cogview} (blurred, machine-translated MS-COCO, best-of-60 re-ranking) and Improved
VQ-Diffusion's 8.44 \citep{image:vqdiffusion-improved} (COCO-trained) are omitted.
$^{\dagger}$\citet{image:unidiffuser} draw 10K and 30K prompts for FID and CLIP score without
...
SEED-X (17B parameters), and ahead of the 12B-parameter DALL$\cdot$E specialist. Every system
with a lower FID than ours uses pretrained language or vision components---a text-pretrained
backbone or text encoder, a pretrained CLIP or VAE---and most also use substantially more paired
data, though several publish no comparable pair count. On GenEval, 0.249 (CC3M) and 0.222
(CC12M) sit above minDALL$\cdot$E and below CoDI and LlamaGen, far below the LLM-backed unified
models. For captioning no zero-shot COCO baseline has
...
tokenizer's own codec contribution ($17.90$ versus $16.39$; CMMD, $18.41\rightarrow17.76$, has no such
...
binding 0.03; Appendix~\ref{app:image:geneval}) and CC12M 0.222. The score is flat in the step
count---0.235 for CC3M at its 10-step image optimum against 0.249 at 256, 0.221 versus 0.222 for
CC12M---so this is the model's compositional ceiling and not a sampler artefact. Captioning is weak in absolute terms (CIDEr 0.136, roughly $7.5\times$
