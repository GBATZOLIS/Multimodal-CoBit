...
\textbf{Limitations.} A dedicated protein study can test that coupling and stronger supervision while testing scalability for biological tasks such as binder--linker design with laboratory (wet lab) validation. CoBit's stronger fold-level ($D_{\mathrm{TM}}$) than coordinate-level ($D_{\mathrm{RMSD}}$) success indicates that denoising structural bits captures useful protein organization information and that geometric precision can be further improved in a dedicated protein-training work. Coordinate-level success varies with protein length as shown in figure~\ref{fig:protein:strict} and although the present metrics indicate modeling quality, establishing biological function requires additional setups and experiments. 
