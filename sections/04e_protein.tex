...
\textbf{Modeling quality versus biological function.} A dedicated protein study can further test scalability to biological tasks and the effectiveness of physical coupling, for example in binder--linker design with laboratory (wet-lab) validation. CoBit's stronger fold-level ($D_{\mathrm{TM}}$) than coordinate-level ($D_{\mathrm{RMSD}}$) success indicates that denoising structural bits captures useful protein organization information and that geometric precision could be further improved through dedicated protein training, including architectural scaling, hyperparameter tuning, distillation, and additional data. In this work, coordinate-level success varies with protein length as shown in figure~\ref{fig:protein:strict} and, although the present metrics indicate modeling quality, establishing biological function is beyond the scope of this work. 
