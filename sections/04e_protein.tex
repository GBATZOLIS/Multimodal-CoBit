...
Unlike text, image, and audio multi-modality, whose correspondence is primarily semantic, a protein sequence and it's three-dimensional structure are physically coupled descriptions. We ask whether the bitstream formulation can learn the coupled modalities of protein sequence and structure by training a 396M-parameter denoiser from scratch on 216,237 PDB/AFDB-SwissProt \citep{protein:pdb, protein:afdb} pairs with 20\% sequence replay. Each residue aligns five amino-acids bits with thirteen signs from the frozen DPLM-2 lookup-free quantiser \citep{wang2024dplm}. Decoding retains the 20 canonical amino acids and all $2^{13}$ structural codes. The common binary interface lets one denoiser lean sequence-structure dependencies while the frozen codec recovers backbone coordinates. Data, splits, and tokenisation are detailed in Appencides \bruno{ref appendx}. 
...
\textbf{Tasks coverage.} Clamping either channel gives inverse or forwarding folding while leaving both free engenders joint generation, without task-specific fine-tuning or Multiple Sequence Alignment as input\bruno{cite MSA?}. Our evaluations were based on entropic DDIM with 50 grid points with conditional CFG 5 and no unconditional guidance (R). Under these settings, mean recovery is 44.10\% on 128 CATH 4.3 validation targets  and forward TM is .554 on 183 CAMEO Targets\citep{protein:cath,protein:cameo,protein:tmscore}. These results test bidirectional learning through one shared bitstream objective. For 80 generated backbones (16 per length, 100--500), one ProteinMPNN redesign and ESMFold refold give mean scTM .899\citep{protein:proteinmpnn,protein:esmfold}. Figure~\ref{fig:protein:multimodal} shows fold similarity across lengths while Figure~\ref{fig:protein:bestgallery} shows the best backbones per length-bucket. We assess backbone recovery after redesign and refolding using scTM  $\geq.9$ ($D_{\mathrm{TM}}$) for overall fold agreement and scRMSD $\leq2$\,\AA{} ($D_{\mathrm{RMSD}}$) for close coordinate agreement. Full metrics and sampling analysis are in Appendices~\bruno{~?}. 
...
\par\addvspace{6pt}\noindent
    ...
    Highest scTM at each length among samples with scRMSD $\leq2$\,\AA{};
...
\textbf{Bitstream capability versus protein optimisation.} We compare CoBit's bitstream formulation against protein-specific models. Table~\ref{tab:protein:baselines} inserts CoBit into the extensive research done by \citep{protein:simpledesign}. MultiFlow \cite{campbell2024multiflow} leads on the strict criteria with a 21.8M-parameters model due to its explicit geometric learning and redesign-select and refolding-filtered training. On the other hand, CoBIt tests the common bit objective with an existing codec and paired-data recipe. Its sequence replay supplies unpaired example unlike MultiFlow's designability distillation or direct geometric supervision. The unmatched comparison cannot isolate representation of parameter efficiency. \bruno{Appendix-X} explains the training differences and MultiFlow's distillation ablation.  
...
Published baselines: \citet{protein:simpledesign}; G: general bitstream framework; P: protein-specific generator.}
...
\vspace{0pt}
\makeatletter\def\@captype{figure}\makeatother
\caption{Visual representation of the rates from Table~\ref{tab:protein:baselines}. CoBit captures protein organisation better than its precise backbone geometry.}
...
A dedicated protein study can test that coupling, stronger supervision, and scaling can be optimised for biological tasks such as binder--linker design with laboratory (wet lab) validation. CoBit's stronger fold-level ($D_{\mathrm{TM}}$) than coordinate-level ($D_{\mathrm{RMSD}}$) success indicates that denoising structural bits capture useful protein organisation information and that geometric precision can be further leveraged in a dedicated protein-training work. The present metrics indicate modeling quality, but biological function requires those improvements and additional experiments. 
...
% \begin{figure}[t]
%     \centering
%     \includegraphics[width=\linewidth]{Multimodal_CoBit/figures/protein/protein_pymol_gallery.pdf}
%     \caption{Placeholder}
