...
\begin{figure}[ht]
    ...
    \caption{(Left) SpkSim and (Right) UTMOS score distributions for text-to-speech for the 39 speakers in LibriSpeech-PC test-clean. For 10 text prompts, we generate audio with CoBit-MLS using held-out speaker tokens from all speakers in the test set. We use NFE=512, $\gamma=0.27$ and gs=5, as selected for standard text-to-speech evaluation.}
