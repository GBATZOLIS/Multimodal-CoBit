...
\midrule
\multicolumn{7}{l}{\emph{CoBit (ours; from scratch, no text pretraining)}} \\
...
\midrule
\multicolumn{8}{l}{\emph{CoBit (ours; from scratch, no text pretraining)}} \\
...
\bottomrule
\end{tabular}
\caption{Text-to-speech and Speech-to-text performance on LibriSpeech-PC. For text-to-speech, evaluation is conducted on the test-clean partition. For each speaker in the dataset, one sample was held out to extract speaker features. Speech-to-text evaluation is conducted on test-clean and test-other partitions separately. As in Table \ref{tab:audio:headline}, Steps$=$512 for both CoBit-LibriTTS and CoBit-MLS.}
...
\midrule
\emph{StableCodec (reconstruction)} & --- & 4.33 & 1{,}191.6 & 1.26 & 0.36 & --- & --- \\
\bottomrule
\end{tabular}%
}
\caption{Continuation-based evaluation for text-audio experiments against published SLMs. All figures are evaluated using publicly available checkpoints. GPT-4o refers to LLM-as-a-Judge score with the prompt shown in Table \ref{tab:audio:llm-judge}. We use Steps$=$512 for both CoBit models. The last row reports the tokenizer performance ceiling.}
...
512 & \textbf{3.5} & \textbf{1.8} & \textbf{3.77} & \textbf{252.4} & \textbf{133.3} \\
\bottomrule
\end{tabular}
...
256 & 5.4 & 5.2 & \textbf{4.03} & \textbf{0.27} & 8.2 & 22.4 \\
...
5 & 3.91 & 2{,}108.4 & 10.6 & 0.34 & \textbf{94.7} & 84.41 \\
\bottomrule
\end{tabular}
...
\midrule
\emph{StableCodec (reconstruction)} & 238.5 & 953.1 & 1{,}191.6 & 0.07 & 1.19 & 1.26 \\
\bottomrule
\end{tabular}%
}
...
Increasing the model scale and adding MLS-English to the training set improves performance across all tasks and metrics. The only exception is FSD-wlm, where CoBit-LibriTTS benefits from its training set being a subset of the LibriSpeech-PC training partition, and is threrefore more similar to the reference population. MLS-English is much larger (45k hours versus 580), preserves reading-style audio and is part of StableCodec's training corpus as well, reducing the risk of distributional shifts during tokenization. CoBit-MLS is better at aligning text and speech sequences during co-generation, yet CoBit-LibriTTS achieves its best error rates when given the text as input. The performance gap is small for SALMON accuracy, as the smaller model can still capture acoustic properties of the speech prompt. Despite its linguistic fluency and frame-level realism, the main limitation of CoBit-LibriTTS is producing natural-sounding audio, observed across all speech-emitting tasks in terms of UTMOS. In generative perplexity, UTMOS, FSD-wlm and SpkSim for speech continuation, CoBit-LibriTTS matches or even outperforms the 395M parameter Flow-SLM version. For LLM-as-a-Judge, however, CoBit-LibriTTS produces almost exclusively irrelevant continuations.  In SALMON, similar to CoBit-MLS, it scores right between multimodal autoregressive SLMs and speech-only models, except its average accuracy is lower than that of Flow-SLM-S.
...
\midrule
\multicolumn{4}{l}{\emph{CoBit (ours; from scratch, no text pretraining)}} \\
...
\textbf{Text-to-speech: speaker realization.} We also evaluate the model's ability to realize different voices in text-to-speech transfer, by generating audio for the same 10 text prompts with all 39 speakers of LibriSpeech-PC test-clean, using CoBit-MLS. Figure \ref{fig:audio:tts-vc} shows how SpkSim and UTMOS scores are distributed across the different speakers. For some speakers, the BiCodec-global representation translates better than others, with speaker similarities ranging from 0.1 to 0.8; UTMOS values further verify samples for all speakers maintain natural sound.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{figures/audio/speaker_tts.png}
    \caption{(Left) SpkSim and (Right) UTMOS score distributions for text-to-speech for the 39 speakers in LibriSpeech-PC test-clean. For 10 text prompts, we generate audio with CoBit-MLS using held-out speaker tokens from all speakers in the test set. We use Steps=512, $\gamma=0.27$ and gs=5, as selected for standard text-to-speech evaluation.}
    \label{fig:audio:tts-vc}
\end{figure}
...
\label{app:protein}
\input{sections/protein_appendix}
