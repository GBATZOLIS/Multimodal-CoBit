...
The $12{\times}512$ denoiser (63.3M parameters) trains for 500K steps with the shared diffusion
objective, entropy controller, self-conditioning and regime probabilities, and receives only the
flattened sequence and its 1-D position. The representation-scale settings
follow the raw-bit sequence rather than CC3M (14 local Fourier features, denominator $P{=}28$);
...
97.90\% on seen tuples (CI $[97.41, 98.30]$) and 97.92\% on held-out tuples (CI
$[97.44, 98.32]$), so the generalisation gap is not statistically distinguishable from zero. The
seen result decomposes into 98.14\% correct perception, 99.66\% correct arithmetic given correct
perception and 0.05\% malformed output.

\textbf{Text$\rightarrow$image} places all four requested digits correctly in 99.98\% of seen
samples (CI $[99.86, 100.00]$) and 99.85\% of held-out samples. This direction is read by a fixed
quadrant classifier, which identifies all four digits in 96.09\% of real seen-pool composites and
99.78\% of its same-pool control; generated digits can be easier to classify than real ones, so
these are instrument calibrations rather than normalisation factors. Without guidance the rate is
99.61\%, and adding stochastic churn gives 99.71\% ($n{=}1{,}024$).

\textbf{Joint co-generation} from noise is 96.85\% mutually consistent (CI $[96.27, 97.34]$). Of
the 4,096 pairs, 99.98\% have a well-formed equation, 99.78\% name the digits the classifier reads
in the generated image and 97.02\% are arithmetically valid, so the residual failures are almost
entirely arithmetic rather than cross-modal.
