...
use that backbone and the two scales differ substantially. CMMD \citep{image:cmmd} and SWD are
...
\textbf{2.35} against the same reference, 99\% of it in the covariance term---finite-sample
estimation, not a train/val shift, which would show in the mean. The same draw after a tokenizer
...
Training steps; relative compute & 1{,}000{,}000; $1\times$ & 1{,}744{,}000 (87\%); $\approx8.7\times$ \\
\midrule
...
FID over a 2.35 sampling floor; the visible distance is between the round-trips and the
generations. Four of the seven fixed protocol captions, one uncurated sample each,
...
\emph{distributional} metric --- it loses CLIP --- and the margin survives floor correction: the shared 2.35 sampling floor cancels and removing each
...
32 steps against 1.39 at 256. On the text side CC3M wins everything by margins far outside the $\sim$0.01 CIDEr noise.
Undertraining predicts a uniform deficit and noisier CC12M alt-text predicts the split observed;
the measured pattern is the second. That is an inference across metrics, not a measurement of
caption quality --- the decisive CMMD-versus-checkpoint curve has not been run.}
...
\caption{What CIDEr 0.136 looks like. Uncurated CoBit-CC3M image$\rightarrow$text samples at the
operating point of Table~\ref{tab:app:image:captioning}. The outputs are fluent CC3M-register
alt-text---the last panel reads ``\emph{modern furniture for a dining table add a table feel to
any space}''---where COCO's references are terse declaratives of the form ``\emph{A dining table
with wooden spoons laid out on it.}'' The content is usually right and the register is not, which is the failure the $n$-gram metrics score
and the CIDEr-versus-RefCLIPScore split in Table~\ref{tab:app:image:captioning} measures over
5{,}000 images. Six uncurated examples, shown to make that failure concrete.}
\label{fig:image:i2t}
\end{figure}
...
We train a 63.3M-parameter $12\times512$ model for 500k steps on $10^6$ examples with the CC3M
recipe unchanged, holding out 10\% of the $10^4$ digit tuples, and evaluate every direction at
$n{=}4{,}096$ with 256 steps. Image$\rightarrow$text is scored by string equality with no
instrument in the loop: 97.90\% of equations are exactly right, decomposing into 98.14\%
perception and 99.66\% arithmetic with 0.05\% malformed output, against a 6.52\% modal-sum
baseline. On digit tuples never seen in training the model scores 97.92\%, indistinguishable from
its in-distribution score (difference $-0.02$, 95\% CI $[-0.65,+0.60]$), so the mapping is learned
rather than memorised. Text$\rightarrow$image, read by a quadrant classifier whose ceiling on real
composites is 99.78\%, places all four digits correctly in 99.98\% of samples. Joint co-generation
reaches 96.85\%, decomposing into 99.98\% well-formed equations, 99.78\% in which the named
addends match the generated image, and 97.02\% in which the arithmetic is valid---so the residual
error is symbolic rather than cross-modal.
...
\textbf{LibriVox-based corpora.} LibriTTS, MLS-English, LibriSpeech-PC and art of SALMON (background and room partitions) are extracted from the LibriVox project, and thus consist of single-speaker, reading-style audiobook recordings. The content CoBit sees during training is exclusively story passages, which severely constraints its stylistic and thematic exposure. Availability of in-the-wild text-audio pairs is very limited at this scale for single-speaker samples, so audiobook data is the natural alternatively. For most speech models, semantic and stylistic generality is inherited from the text pre-trained backbone. Note that the training and test sets have no example or speaker overlap. 
...
Tables \ref{tab:audio:joint}, \ref{tab:audio:cross-modal} and \ref{tab:audio:continuation} contain extensive evaluation metrics and baseline comparisons for co-generation, cross-modal directions and continuation, respectively; Table \ref{tab:audio:salmon} details per-task SALMON accuracy. A mel-spectrogram example for speech continuation is visualized in Figure \ref{fig:audio:continuation-spectrogram}.
...
\bottomrule
\end{tabular}
\caption{Continuation-based and SALMON evaluation for text-audio experiments against published SLMs. All figures are evaluated using publicly available checkpoints. GPT-4o refers to LLM-as-a-Judge score with the prompt shown in Table \ref{tab:audio:llm-judge}. We use NFE$=$512 for both CoBit models. The last row reports the tokenizer performance ceiling.}
...
Shuffled & 0.26 ($\pm$0.03) & 10.58 ($\pm$0.03) & 10.99 ($\pm$0.003) & 0.222 ($\pm$0.0004) \\
\bottomrule
\end{tabular}
\caption{Speaker conditioning realization for CoBit-MLS co-generation. Comparisons are drawn between BiCodec-global representations CoBit-MLS samples directly (speaker positions in its generated sequence) and BiCodec-global representations extracted from the waveforms CoBit-MLS generates. RMSE and cosine similarity metrics are computed on BiCodec-global's latent space by transforming each FSQ code into its underlying latent vector. The first row computes paired sample correspondance (generated and realized representations for the same sequence); the second row gives a performance floor for shuffled examples, averaged over 100 draws. Cosine similarity is averaged over sequence positions (32) and the generated population (1024 samples).}
\label{tab:audio:realized-conditioning}
\end{table}

\textbf{Co-generation: speaker diversity.} For co-generation, we are also interested in whether CoBit can actually learn to draw random voices, instead of repeating a small set of identities. To test this, we calculate the similarity among generated populations under an external speaker verification system that is unrelated with ECAPA-TDNN and BiCodec, namely CAM++ \citep{audio:campp}. We compare CoBit-LibriTTS and CoBit-MLS with with the cascaded joint generation pipeline (Appendix \ref{app:audio:baselines}), which learns to autoregressively sample speaker tokens using the same training set and representation as CoBit-MLS and realizes speaker conditioning via the BiCodec decoder. Results are reported in Table \ref{tab:audio:diversity} and the distributions of within-sample cosine similarity are visualized in the bottom panel of Figure \ref{fig:audio:speaker}. CoBit-MLS generates as diverse speakers as the baseline on average, with significant distribution overlap; CoBit-LibriTTS, having been trained on a much narrower speaker collection, is heavily limited in the voices it can reproduce.
...
CoBit-LibriTTS & 8.64 & 0.27 & 0.295 \\
CoBit-MLS & --- & --- & 0.439 \\
\midrule
...
Matched Population & 8.75 ($\pm$0.1) & 0.3 ($\pm$0.002) & --- \\
Full & 9.97 & --- & ---\\
\bottomrule
\end{tabular}
\caption{Co-generation speaker diversity, measured in terms of speaker tokens entropy and cosine similarity of the sampled BiCodec-global latents and the extracted CAM++ embeddings. Matched population statistics for the training set are computed using 100 random draws.}
