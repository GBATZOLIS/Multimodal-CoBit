...
objective introduced in this work.

\subsection{Continuous bitstream diffusion}

CoBit uses a variance-exploding Gaussian diffusion with
$\sigma\in[0.002,80]$, data centre $1/2$ and $\sigma_{\mathrm{data}}=1/2$. The analytic posterior
...
steps $4$--$5\times10^4$, and refreshes the estimate every $2\times10^3$ steps.
...
step without another evaluation. The unused \texttt{refined} alternative recomputes the estimate
...
are also copied into $\hat\vx_0$. For classifier-free guidance, conditioning segments are replaced
